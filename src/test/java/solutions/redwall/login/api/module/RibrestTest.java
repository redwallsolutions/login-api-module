...
public class RibrestTest {
    ...
    protected String getFirstExistentModel(String jsonObject) {
        return String.valueOf(new JSONObject(jsonObject).getJSONObject("holder").getJSONArray("models").getJSONObject(0).getLong("id"));
    }

    protected MultivaluedMap<String,String> getNewMVMInstance() {
    ...
    public void delete(Class resource, String subpath, Form form) {
        WebTarget wt = buildWebTarget(resource, subpath);
        response = deleteResponse(resource, subpath, form);
        getResponseText();
    }

    public Response deleteResponse(Class resource, String subpath, Form form) {
        WebTarget wt = buildWebTarget(resource, subpath);
